...
print("Loading data...")
try:
    df_berlin = pd.read_csv(berlin_file_location, parse_dates=["timestamp"]) # parse_dates stores the column as a datetime variable
...
# --- Basic dataset info ---
print("=== BERLIN DATA ===")
print("Shape:", df_berlin.shape) # Tells the dimension of data for example 100x4
print("Columns:", df_berlin.columns.tolist())
print(df_berlin.dtypes) # Tells the type of each column
print()
...
print(df_berlin.isna().sum()) # .isna returns True (1) for each missing cell, .sum totals them up giving us total missing
print()
...
# .T method is used to transpose the dataset in order to increase readability
print()
...
# In my experience, regular indexing (i.e 0 - time - data did not work)
try:
...
# resample() is used when your DataFrame has a datetime index.
# 'ME' stands for Month-End — meaning it groups data by each month, ending on the last day of the month.

# --- Give the Series a name (this names a new column) ---
...
# --- Set the index name (this names the index column since we want to easily see the monthly averages) ---
...
try:
    ...
    # --- Find common limits for a fair comparison ---, so we can easily visually compare data
...
def plot_wind_rose(df, ax, title):
    ...
    # We use arctan2(u, v) and add 180 to get the "coming from" direction
    # arctan2 is better for this application as it's more polished
    # 0=North, 90=East, 180=South, 270=West
    direction_rad = np.arctan2(df['u10m'], df['v10m']) # We get the "vector angle" with the arctan calculation
    direction_deg = (np.degrees(direction_rad) + 180) % 360 # We divide by 2pi
    # We add +180 because this function normally returns values between -180 and +180
    ...
    # This pd.cut function splits directional degrees into these bins and assigns each value a corresponding interval label.
    ...
    # Same story with the above example
    ...
    # Creates a cross-tabulation table (like a pivot table), storing matching directions and speeds for graph creation
...
# I am not going to lie, I knew this was a cool idea but had no idea how to implement it
# I fully used AI to write this and could not fully understand some parts
